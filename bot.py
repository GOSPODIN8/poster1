...
async def main():
    ...
    if not os.path.exists(content_generator.REFERENCE_IMAGE_PATH):
        logger.error(f"Референс-фото не найдено: {content_generator.REFERENCE_IMAGE_PATH}")
        if config.ADMIN_ID:
            try:
                await bot.send_message(
                    config.ADMIN_ID,
                    f"⚠️ При запуске не найден файл {content_generator.REFERENCE_IMAGE_PATH} — "
                    f"проверь, что папка assets/ загружена в проект на Railway. Пока картинок не будет.",
                )
            except Exception:
                pass
